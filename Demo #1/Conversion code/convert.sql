	...
	@JsonData nvarchar(max) = '
		{
			"installed_apps": [{
			  "name": "Spotify",
			  "added_timestamp": 1513053219
			}]
		}',
	-- double check to see if it is the same as db
...
WITH jsonRoot AS (
	SELECT 
		0 as parentLevel, 
		CONVERT(nvarchar(4000),NULL) COLLATE Latin1_General_BIN2 as parentTableName, -- double check COLLATE to see if it is the same as db
		0 AS [level], 
		[type] ,
	...
	SELECT 
		...
		CONVERT(nvarchar(4000),jsonRoot.TableName) COLLATE Latin1_General_BIN2, -- double check COLLATE to see if it is the same as db
		jsonRoot.[level]+1, 
		d.[type],
...
-- debugging: SELECT * FROM ##parsedJson ORDER BY ParentLevel, level, tablename, columnsequence
